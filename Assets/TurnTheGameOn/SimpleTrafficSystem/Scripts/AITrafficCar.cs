namespace TurnTheGameOn.SimpleTrafficSystem
{
    ...
    public class AITrafficCar : MonoBehaviour
    {
        ...
        {
            if (onReachWaypointSettings.parentRoute != AITrafficController.Instance.GetCarRoute(assignedIndex)) return;

            PlayerCar playerCar = GetComponent<PlayerCar>();
            AITrafficWaypoint waypoint = onReachWaypointSettings.parentRoute.waypointDataList[onReachWaypointSettings.waypointIndexnumber - 1]._waypoint;
            ...
            // If there are new routes available at this node
            if (onReachWaypointSettings.newRoutePoints.Length > 0)
            {
                ...
                // If it's the last point in the list, continue with any route available
                if (onReachWaypointSettings.waypointIndexnumber == onReachWaypointSettings.parentRoute.waypointDataList.Count)
                {
                    ...
                    AITrafficWaypoint nextTrafficWaypoint = playerCar
                        ? playerCar.GetNextTrafficWaypoint(waypoint, onReachWaypointSettings.newRoutePoints)
                        : onReachWaypointSettings.newRoutePoints[randomIndex];

                    AITrafficController.Instance.Set_WaypointRoute(assignedIndex, nextTrafficWaypoint.onReachWaypointSettings.parentRoute);
                    AITrafficController.Instance.Set_RouteInfo(assignedIndex, nextTrafficWaypoint.onReachWaypointSettings.parentRoute.routeInfo);
                    AITrafficController.Instance.Set_RouteProgressArray(assignedIndex, nextTrafficWaypoint.onReachWaypointSettings.waypointIndexnumber - 1);
                    AITrafficController.Instance.Set_CurrentRoutePointIndexArray
                        (
                        assignedIndex,
                        nextTrafficWaypoint.onReachWaypointSettings.waypointIndexnumber - 1,
                        nextTrafficWaypoint
                        );
                }

                // If there is a new route available for this car type, potentially take it
                else if (newRoutePointsMatchingType.Count > 0)
                {
                    ...
                    AITrafficWaypoint[] filteredWaypoints = new AITrafficWaypoint[newRoutePointsMatchingType.Count];
                    for(int i = 0; i < newRoutePointsMatchingType.Count; i++)
                        filteredWaypoints[i] = onReachWaypointSettings.newRoutePoints[newRoutePointsMatchingType[i]];

                    AITrafficWaypoint nextTrafficWaypoint = playerCar
                        ? playerCar.GetNextTrafficWaypoint(waypoint, filteredWaypoints)
                        : filteredWaypoints[randomIndex];
            ...
            // If this is not the last node, continue on this route
            ...
            // Update the list in the traffic controller
            AITrafficController.Instance.Set_RoutePointPositionArray(assignedIndex);
            // Delay continue driving if this node has a stop time
